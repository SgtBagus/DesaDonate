  ...
  <div class="modal fade bd-example-modal-sm" tabindex="-1" produk="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" id="modal-delete">
      <div class="modal-dialog modal-sm">
          <div class="modal-content">
              <form id="upload-delete" action="<?= base_url('master/Produk/delete') ?>">
  ...
  <div class="modal fade" id="modal-impor">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form action="<?= base_url('fitur/impor/produk') ?>" method="POST"  enctype="multipart/form-data">
  ...
  <script type="text/javascript">
        ...
        function loadtable(status) {
            var table = '<table class="table table-bordered" id="mytable">'+
                   '     <thead>'+
                   '     <tr>'+
                   '       <th style="width:20px">No</th>'+'<th>Nama</th>'+'<th>Tanggal</th>'+'<th>Gambar</th>'+'       <th style="width:150px">Status</th>'+
             ...
             $("#load-table").html(table)
              ...
              var t = $("#mytable").dataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: {"url": "<?= base_url('master/Produk/json?status=') ?>"+status, "type": "POST"},
                columns: [
                    {"data": "id","orderable": false},{"data": "nama"},{"data": "tanggal"},{"data": "gambar"},
                   {"data": "status"},
                    {   "data": "view",
                        "orderable": false
                    }
                    ...
                    { targets : [4],
                        render : function (data, type, row, meta) {
                              if(row['status']=='ENABLE'){
                                var htmls = '<a href="<?= base_url('master/Produk/status/') ?>'+row['id']+'/DISABLE">'+
                                            '    <button type="button" class="btn btn-sm btn-sm btn-success"><i class="fa fa-home"></i> ENABLE</button>'+
                                            '</a>';
                              }else{
                                var htmls = '<a href="<?= base_url('master/Produk/status/') ?>'+row['id']+'/ENABLE">'+
         ...
         function edit(id) {
            location.href = "<?= base_url('master/Produk/edit/') ?>"+id;
         }
